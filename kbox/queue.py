...
from .models import SongMetadata, SongSettings, QueueItem, User
...
class QueueManager:
        ...
        if item.created_at:
            if datetime.now(item.created_at.tzinfo) - item.created_at > self._download_timeout:
                self.logger.warning(
                    ...
                    item.metadata.title, item.id, self._download_timeout
                )
    ...
    def add_song(
        ...
        self.logger.info('Added song to queue: %s by %s (ID: %s, source: %s, pitch: %s)', 
                       title, user.display_name, item_id, source, pitch_semitones)
        return item_id
